...
10. 추가 기능
    - [x]  User의 역할이 Admin일 경우 어떠한 예외도 반환하지 않는다. -> 검증에 성공한다.
    - [x]  User의 역할이 Clinet일 경우 `UnauthroizedAccessException`을 반환한다. -> 검증에 실패한다.
    - [x]  총 판매 금액 계산한다.
    - [x]  12월 이벤트 참여 유저 수를 계산한다.
    - [x]  설날 이벤트에 재참여할 것이라 기대되는 유저 수를 계산한다. -> 5%의 수
    - [x]  `eventService`와 `userService`의 `Users`의 인스턴스가 같은지 확인한다. -> 싱글톤 확인