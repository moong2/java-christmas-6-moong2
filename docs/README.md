...
## 테스트 목표
1. 식당 예상 방문 날짜
   - [ ]  입력값이 숫자인 경우 어떠한 에러도 반환되지 않는다.
   - [ ]  입력값이 숫자가 아닌 경우 `IllegalArgumentException`이 반환되며 에러 메시지는 `[ERROR] 유효하지 않은 날짜입니다. 다시 입력해 주세요.`이다.
   - [ ]  입력값이 1이상 31이하일 경우 어떠한 에러도 반환되지 않는다.
   - [ ]  입력값이 1이상 31이하가 아닐 경우 `IllegalArgumentException`이 반환되며 에러 메시지는 `[ERROR] 유효하지 않은 날짜입니다. 다시 입력해 주세요.`이다.